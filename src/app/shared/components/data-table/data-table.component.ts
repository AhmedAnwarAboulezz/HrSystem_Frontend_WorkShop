...
export class DataTableComponent implements OnInit, OnDestroy {
  ...
  @Output() assignDetail: EventEmitter<any> = new EventEmitter<any>();
  ...
  saveColumnsOrder(event: { columns: any; }): void {
    this.Storage.set(this.tableData.componentName, JSON.stringify(event.columns));
  }
  ...
  showOrHideColumns(column: ColumnsInterface) {
    ...
    this.Storage.set(this.tableData.componentName, JSON.stringify(this.selectedColumns));
  }
  ...
  /**
   * Search Filter For Each Table Column
   */
  ...
  openAssignDetail(row?: any) {
    this.assignDetail.emit(row);
  }
      ...
      this.selectedColumns.forEach(e => {
        if (e.print) {
          ...
          if (data[fieldName] === false) {
            if(e.filterMode == "status") row.push(this.localize.translate.instant('Offline'));
            else{
            ...
            if(e.filterMode == "status") row.push(this.localize.translate.instant('Online'));
            else{
  ...
  async exportPdf() {
    let organizationsData = this.Storage.get('Organizations_data');
